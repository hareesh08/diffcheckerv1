...
export function UploadScreen({
  ...
  onLoaded: (a: UploadedFile | null, b: UploadedFile | null) => void;
}) {
  ...
  function removeFile(side: Side) {
    setFiles((f) => ({ ...f, [side]: undefined }));
  }
  ...
  return (
    <div className="flex-1 overflow-auto">
      <div className="mx-auto max-w-5xl px-6 py-8">
        {error && (
          <div className="mb-4 rounded-md border border-diff-del/30 bg-diff-del/8 px-3 py-2 text-xs text-foreground">
            {error}
          </div>
        )}
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-lg font-semibold tracking-tight">New comparison</h1>
            <p className="mt-0.5 text-xs text-muted-foreground">
              Load two sources to compute a cell-level difference.
            </p>
          </div>
            ...
            {(["excel", "text"] as const).map((t) => (
              <button
                ...
                className={cn(
                  ...
                  tab === t
                    ? "bg-surface text-foreground shadow-sm ring-1 ring-hairline"
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {(["a", "b"] as const).map((side) => (
                <div
                  key={side}
                  className="flex h-56 flex-col items-center justify-center gap-2 rounded-md border border-dashed border-border bg-surface transition-colors hover:border-muted-foreground hover:bg-card"
                ...
                >
                  <label className="flex h-full w-full cursor-pointer flex-col items-center justify-center gap-2">
                    ...
                    />
                    <div className="grid size-10 place-items-center rounded border border-hairline bg-grid font-mono text-[10px] text-muted-foreground">
                      XLS
                    </div>
                ...
                {(["a", "b"] as const).map(
                  (side) =>
                    files[side] && (
                      ...
                      >
                        <div className="grid size-8 place-items-center rounded border border-hairline bg-grid font-mono text-[10px] text-muted-foreground">
                          XLS
                        </div>
            ...
            <div className="flex justify-end">
              <button
                type="button"
                disabled={!files.a || !files.b}
                onClick={() => onLoaded(files.a ?? null, files.b ?? null)}
                className="rounded-md bg-primary px-3 py-1.5 text-xs font-semibold text-primary-foreground shadow-sm transition-colors hover:bg-primary/90 disabled:opacity-40"
              >
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <textarea
                ...
                spellCheck={false}
                className="h-56 resize-none rounded-md border border-border bg-grid p-3 font-mono text-[12px] leading-relaxed outline-none focus:ring-1 focus:ring-ring"
              />
              <textarea
                value={right}
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => setResult(diffLines(left, right))}
            ...
            {result && (
              <div className="overflow-hidden rounded-md border border-border bg-grid">
                <table className="w-full border-separate border-spacing-0 font-mono text-[12px]">
                  <tbody>
                    {result.map((line, i) => (
                      <tr key={i} className={rowTone[line.status]}>
                        <td className="w-10 border-r border-hairline bg-grid-header px-2 py-1 text-right text-[10px] text-muted-foreground">
                          {line.leftNo ?? ""}
                        </td>
                        <td className="w-1/2 border-r border-hairline px-3 py-1 whitespace-pre-wrap">
                          {line.left ?? ""}
                        </td>
                        <td className="w-10 border-r border-hairline bg-grid-header px-2 py-1 text-right text-[10px] text-muted-foreground">
                          {line.rightNo ?? ""}
