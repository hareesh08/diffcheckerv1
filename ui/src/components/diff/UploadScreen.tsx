...
const rowTone = {
  same: "",
  modified: "bg-neon-yellow/8",
  added: "bg-neon-green/8",
  deleted: "bg-neon-red/8",
} as const;
  ...
  return (
    <div className="flex-1 overflow-auto">
      <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
        {error && (
          <div className="mb-4 rounded border border-neon-red/30 bg-neon-red/5 px-3 py-2 text-xs text-foreground">{error}</div>
        )}
          ...
          <div className="flex w-full rounded border border-border bg-background p-0.5 sm:w-auto">
        ...
        {tab === "excel" ? (
          <div className="space-y-4">
            <div className="rounded border border-neon-green/30 bg-neon-green/5 px-3 py-2 text-xs text-foreground">
              Compare Excel files and other spreadsheets - .xlsx, .xlsm, .csv, .tsv, .txt
            </div>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {(["a", "b"] as const).map((side) => (
                <div key={side}
                  className="flex h-52 flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-border bg-card transition-all hover:border-neon-blue/60 sm:h-60"
                  ...
                  <label className="flex h-full w-full cursor-pointer flex-col items-center justify-center gap-2">
                    ...
                    <div className="grid size-12 place-items-center rounded-lg border border-border bg-background font-mono text-[10px] font-bold text-foreground">XLS</div>
            ...
            {(files.a || files.b) && (
              <div className="space-y-2">
                {(["a", "b"] as const).map((side) =>
                  files[side] && (
                    <div key={side} className="flex items-center gap-3 rounded border border-border bg-card px-3 py-2">
                      <div className="grid size-8 shrink-0 place-items-center rounded border border-border bg-background font-mono text-[10px] text-muted-foreground">XLS</div>
            ...
            <div className="flex justify-end">
              <button type="button" disabled={!files.a || !files.b}
                ...
                className="w-full rounded bg-foreground px-3 py-2 text-xs font-semibold text-background transition-opacity hover:opacity-90 disabled:opacity-40 sm:w-auto sm:py-1.5">
                Review comparison
              </button>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <textarea value={left} onChange={(e) => setLeft(e.target.value)} spellCheck={false}
                className="h-40 resize-none rounded border border-border bg-card p-3 font-mono text-[12px] leading-relaxed outline-none focus:ring-1 focus:ring-ring sm:h-56" />
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button type="button" onClick={() => setResult(diffLines(left, right))}
                className="rounded bg-foreground px-3 py-2 text-xs font-semibold text-background transition-opacity hover:opacity-90 sm:py-1.5">
                Find difference
              </button>
              <button type="button" onClick={() => { setLeft(right); setRight(left); }}
                className="rounded border border-border bg-background px-3 py-2 text-xs font-medium text-muted-foreground transition-colors hover:text-foreground sm:py-1.5">
                Swap
              </button>
